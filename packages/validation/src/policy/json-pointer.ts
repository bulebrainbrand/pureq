...
export const encodeJsonPointer = (tokens: readonly string[]): string => {
  if (tokens.length === 0) {
    return "/";
  }
...
export const decodeJsonPointer = (pointer: string): string[] => {
  if (pointer === "/") {
    return [];
  }
...
const toTokensFromLoosePath = (path: string): string[] => {
...
export const normalizePathToJsonPointer = (path?: string): string => {
